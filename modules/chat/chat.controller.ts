...
const extractMentions = (content: string): string[] => {
  // Match @username patterns - captures word characters including underscores
  // Multi-word names use underscores (e.g., @Vansh_Khaneja)
...
const findUsersByMention = async (projectId: string, mentionNames: string[]): Promise<string[]> => {
  ...
  for (const name of mentionNames) {
    // Replace underscores with spaces for matching (mentions use underscores for multi-word names)
    const normalizedName = name.replace(/_/g, ' ').toLowerCase();
    const member = members.find(
      (m) => m.users?.name?.toLowerCase() === normalizedName ||
             m.users?.name?.toLowerCase().includes(normalizedName)
    );
    if (member?.user_id) {
